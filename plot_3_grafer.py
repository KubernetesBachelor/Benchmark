...
df3 = pd.read_csv('./parset_cpu_resultater_Native.csv')  # Assuming tab-delimited; adjust if needed

# Determine all combinations of threads and cpu-max-prime in the datasets
combinations = pd.concat([df1[['Threads', 'cpu-max-prime']], df2[['Threads', 'cpu-max-prime']], df3[['Threads', 'cpu-max-prime']]]).drop_duplicates()
...
for _, row in combinations.iterrows():
    ...
    # Find the highest y-value across all datasets
    max_y_value = max(data1['Total Number of Events'].max(), data2['Total Number of Events'].max(), data3['Total Number of Events'].max())
    
    # Calculate 2/3 of the maximum y-value
    min_y_value = 1/3 * max_y_value

    # Create a new figure for the plot
    plt.figure(figsize=(10, 6))
    
    # Plot data from all three files
    ...
    plt.plot(data3['Test Number'], data3['Total Number of Events'], label='Native', marker='x')  # Different marker for distinction
    ...
    plt.axhline(y=avg3, color='green', linestyle='--', label=f'Native Gjennomsnitt: {avg3:.2f}')  # New line for the third average
